...
from adw_modules.data_types import (AgentCall, BuildOutput, DocumentOutput,
                                    GenericOutput, PhaseParams, PlanOutput,
                                    ReviewOutput, ScoutOutput)
...
def main(prompt: str, agent: str = "builder",
    ...
    with run.phase(PhaseParams(name="prompt", kind="agent", owner=agent,
        ...
        ph.call(AgentCall(output_type=OUTPUT_TYPES.get(agent, GenericOutput), prompt=prompt))
