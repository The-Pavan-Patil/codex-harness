...
class Console:
    ...
    def session_finished(self, ok: bool, tokens: int, cost: float | None, db_path: str) -> None:
        ...
        rows = [f" [dim]status[/dim]   {status}",
                ...
                f" [dim]cost[/dim]     {f'${cost:.4f}' if cost is not None else 'unavailable (subscription)'}",
        ...
        cost_text = f"${cost:.4f}" if cost is not None else "cost unavailable"
    ...
    def agent_finished(self, name: str, tokens: int, cost: float | None) -> None:
        cost_text = f"${cost:.4f}" if cost is not None else "subscription cost unavailable"
        self._emit(f"  [dim]└ {escape(name)} used {tokens:,} tokens · {cost_text}[/dim]")
