...
def commit_all(message: str) -> str:
    """Stage the factory's clean-baseline working tree and commit it."""
    if not is_repo():
        raise RuntimeError(
...
def require_clean_worktree() -> None:
    """Fail before a commit-producing ADW can absorb operator-owned changes.

    Commit workflows intentionally use ``git add -A`` because their preceding
    phases may create, modify, rename, or delete arbitrary product files. That
    is safe only when the run starts clean. Call this before opening the first
    phase, not at commit time after the factory has created legitimate dirt.
    """
    if not is_repo():
        raise RuntimeError(
            "this workflow ends in a commit and requires a Git repository")
    dirty = _git("status", "--porcelain")
    if dirty:
        preview = "\n".join(dirty.splitlines()[:20])
        raise RuntimeError(
            "commit-producing ADWs require a clean working tree so `git add -A` "
            f"cannot include operator changes. Commit/stash them first:\n{preview}")
