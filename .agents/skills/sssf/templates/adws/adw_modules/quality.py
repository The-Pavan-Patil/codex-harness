...
║                                                                              ║
║  Every block ships UNCONFIGURED and fails closed with exit 78. A stamped      ║
║  repo cannot guess your test runner, and a fake green run is worse than an     ║
...
def _placeholder(name: str) -> list[str]:
    """An empty argv is an explicit, fail-closed unconfigured check."""
    return []
...
def _run(spec: QualityCheckSpec, run) -> QualityCheckResult:
    ...
    command = shlex.join(spec.argv) if spec.argv else f"UNCONFIGURED:{spec.name}"
    ...
    try:
        if not spec.argv:
            returncode = 78
            stderr = (f"Quality check {spec.name!r} is not configured. Edit "
                      "adws/adw_modules/quality.py and provide a real argv list.")
            completed = None
        else:
