...
class Run:
    def __init__(self, cfg, adw_id: str, tracer, engineer: str):
        ...
        self.tokens = 0
        # ChatGPT subscription turns expose tokens but no reliable per-run USD
        # amount. ``None`` means unavailable; deterministic code-only runs may
        # remain at zero until an agent turn occurs.
    ...
    def add_usage(self, tokens: int, cost: float | None) -> None:
        self.tokens += tokens
