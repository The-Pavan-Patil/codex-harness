...
class Tracer:
    ...
    def session_add_usage(self, adw_id: str, tokens: int, cost: float | None) -> None:
        if cost is None:
            self.conn.execute(
                "UPDATE sessions SET total_tokens=total_tokens+?, total_cost=NULL WHERE adw_id=?",
                (tokens, adw_id),
            )
            return
        self.conn.execute(
            "UPDATE sessions SET total_tokens=total_tokens+?, "
            "total_cost=COALESCE(total_cost, 0)+? WHERE adw_id=?",
            (tokens, cost, adw_id),
        )
