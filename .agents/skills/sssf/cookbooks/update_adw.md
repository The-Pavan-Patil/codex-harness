...
Any ADW with a commit phase must call `git_helper.require_clean_worktree()` before `session.ensure()`. The commit helper intentionally stages the entire factory-produced working tree with `git add -A`; the preflight is what prevents it from absorbing pre-existing operator changes. `commit_message` is a field on `PlanOutput`, `BuildOutput`, and `DocumentOutput` that the agent fills in **for its own work product**, so always pair it with a fallback — it defaults to empty. `commit_all` raises if the cwd is not a git repo or nothing changed, which fails the phase rather than committing nothing. A chain that commits more than once (`adw_simple_sdlc.py`) commits each product with its own author's message.
...
On violations the harness does **not** restart the agent — it sends the violation list back into the **same Codex thread** with `exec resume`, bounded by that phase's `retries`. Every gate result is traced to `gate_results`. Exhausting the retries raises `GateFailure` and fails the phase.
