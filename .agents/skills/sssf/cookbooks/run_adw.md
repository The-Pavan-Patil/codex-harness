...
adws/adw_sssf_config/sssf.config.yaml
    planner     gpt-5.6
    builder     gpt-5.6-terra (inherited)
...
| "have the frontier roster plan this one" | the named Codex roster; if none exists, say so rather than editing the config mid-request |
...
`processes` rows with `ended_at IS NULL` are live. If `procs` shows a Codex child but `raw_output.jsonl` is empty, check `codex_stderr.log`, `codex login status`, project trust, hook trust, the selected model, and the permission profile. `just kill` verifies each pid still matches the recorded command before signalling because pids get recycled.
