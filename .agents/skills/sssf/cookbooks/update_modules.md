...
| Module | Owns |
|---|---|
| `data_types.py` | Every Pydantic model: calls, phases, envelopes, config, events, `CodexRequest`, `CodexResult`, and usage |
| `agents.py` | `load_config`, `validate`, role resolution, strict output schemas, permission translation, and typed Codex execution |
| `runner.py` | the `Run` object; `run.phase(PhaseParams)` context manager; `ph.call(AgentCall)` |
| `agent_codex.py` | Codex CLI interface — `exec --json --output-schema`, resume by thread id, event normalization, hook spool, and subscription token usage |
| `codex_hook.py` | append trusted native-subagent lifecycle records to the per-role hook spool |
