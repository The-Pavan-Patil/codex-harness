...
1. **What agents, in what order?** Pick from the roster (`adws/adw_sssf_config/sssf.config.yaml`). The five starter roles cover most chains:
...
- `kind="agent"` → `owner` MUST be an agent name from the config — it selects the Codex model, reasoning, permission profile, prompts, and lane. `ph.call()` runs whoever owns the phase.
...
- `retries=N` on an **agent** phase = extra gate-correction rounds sent through `codex exec resume <thread-id>`, so context stays intact. Code-phase re-execution is not automatic.
...
def main(prompt: str, config: str = "adws/adw_sssf_config/sssf.config.yaml", adw_id: str | None = None) -> int:
    ...
    git_helper.require_clean_worktree()         # 3. required because this chain commits with add -A
    run = session.ensure(cfg, adw_id)           # 4. pin-or-create the session → the Run object
...
- **Committing is a code phase, and it needs a clean preflight plus a fallback.** Call `git_helper.require_clean_worktree()` before `session.ensure()` so `git add -A` cannot absorb operator-owned changes. `PlanOutput`, `BuildOutput`, and `DocumentOutput` each carry a `commit_message` the agent writes **for its own work product** — the spec, the code, the write-up. It defaults to empty, so always `envelope.commit_message or <fallback>`, and commit each product with the message of the agent that made it (`adw_simple_sdlc.py` commits three times and never crosses them). `git_helper.commit_all(message)` stages everything, commits, and returns the short sha; it raises a clear error when the cwd isn't a git repo or nothing changed, and that raise fails the phase.
