...
│   ├── agent_codex.py           Codex exec/resume, JSONL, schemas, hooks, token usage
...
**The runtime is Codex CLI only.** `coding_agent: codex`, ChatGPT subscription authentication from `codex login`, structured JSON output, and resumable per-role thread ids. Project `.codex` permission profiles constrain writes while the existing Git-content checker verifies them afterward.
