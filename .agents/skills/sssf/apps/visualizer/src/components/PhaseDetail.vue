...
const phaseUsage = computed<{ rows: UsageRow[]; partial: boolean } | null>(() => {
  ...
  if (u.reasoning_tokens) {
    ...
    const share = u.output_cost != null && u.output_tokens
      ? (u.output_cost * u.reasoning_tokens) / u.output_tokens
      : null
    rows.push({
      label: 'thinking',
      ...
      <div class="d-col">
        ...
        >
          <div class="cfg">
            ...
            <div v-if="agentConfig.permission_profile" class="cfg-row">
              <span class="cfg-k">permissions</span>
              <span class="cfg-chip">{{ agentConfig.permission_profile }}</span>
            </div>
            <div v-if="agentConfig.subagents !== undefined" class="cfg-row">
              <span class="cfg-k">subagents</span>
              <span class="cfg-v">{{ agentConfig.subagents ? 'native Codex agents enabled' : 'disabled' }}</span>
            </div>
