...
Reusable combination of **agents plus code**: deterministic Python ADW scripts own sequencing, retries, and acceptance; Codex CLI agents authenticated through the operator's ChatGPT subscription work inside bounded phases; typed JSON envelopes carry context between them; everything streams into SQLite for the polled visualizer. Agent proposes, code disposes.
...
9. **`tools:` documents intent; permissions are the boundary** — Codex's shell can run anything allowed by the sandbox, so a tool list cannot make "this agent changes nothing" true. Project `.codex` permission profiles enforce the boundary during execution, while `writes:` and `protected_files` are independently content-hashed in `adw_modules/permissions.py` after every call. Unauthorized changes are restored and the phase dies. The session runtime under `data_dir` stays writable so read-only roles can still hand off reports.
...
Codex CLI only (`coding_agent: codex`), using authentication saved by `codex login`; no API key or separate harness runtime is required. `codex exec --json` supplies the streamed event transport, `--output-schema` supplies the typed-response contract, and `exec resume <thread-id>` preserves context across corrections and joined ADWs. Planner and scout may use native Codex subagents; all primary workflow sequencing remains deterministic Python. Subscription token usage is recorded, while per-run USD cost is explicitly unavailable.
