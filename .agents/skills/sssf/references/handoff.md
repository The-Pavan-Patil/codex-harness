...
The first Codex call records its `thread.started` id. Every parse or gate
correction then uses `codex exec resume <thread-id>`, so context stays intact.
`--output-schema` requests the Pydantic-derived shape before the response is
validated again, and every failed attempt is persisted as an invalid envelope.
...
```json
{
  ...
  "findings": [
    ...
    "notes_for_next_agent": "<anything the next role should verify>"
}
```
````
...
└── {agent_name}/
    ...
    ├── codex_stderr.log    CLI diagnostics, separate from machine JSONL
    ├── codex_hooks.jsonl   trusted native-subagent lifecycle records
    ├── output_schema.json  exact schema requested for the call
...
```json
{
  "planner": {"session_id": "0199a213-81c0-7800-8aa1-bbab2a035a53",
              "model": "gpt-5.6", "coding_agent": "codex",
              "permission_profile": "sssf-planner"},
  "builder": {"session_id": "0199a214-20ad-7e10-bf64-d104caf10c08",
              "model": "gpt-5.6-terra", "coding_agent": "codex",
              "permission_profile": "sssf-builder"}
}
```
...
The map records the backend, model, and permission profile. Changing any of
them starts a **fresh** thread for that role. `agent_sessions` in `sssf.db` is
the queryable mirror of this file.
